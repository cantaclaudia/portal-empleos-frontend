...
import { useReclutadorContext } from '../contexts/ReclutadorContext';
...
const JobCard = ({ title, applications, location, description, salary, publishedDate }: JobCardProps): JSX.Element => (
  <div className="bg-white rounded-[12px] shadow-md p-4 md:p-8 hover:shadow-lg transition-shadow duration-200 border border-gray-200">
    ...
    <div className="flex items-center gap-2 mb-4 text-[#666666]">
      ...
      <span className="text-[16px] leading-[22.4px]">{location}</span>
    </div>
    <p className="text-[#333333] text-[16px] leading-[24px] mb-4">{description}</p>
    <div className="flex flex-col md:flex-row md:items-center md:justify-between pt-4 border-t border-gray-200 gap-4">
      <div className="flex flex-col gap-1">
        <span className="font-semibold text-[#05073c] text-[16px]">{salary}</span>
...
const ManagementCard = ({ icon: Icon, title, count, description }: ManagementCardProps): JSX.Element => (
  <div className="w-full max-w-[1194px] bg-white rounded-[12px] shadow-md hover:shadow-lg transition-all duration-200 p-4 md:p-8 flex items-center gap-4 md:gap-6 border border-gray-200 cursor-pointer hover:border-[#f46036]">
    ...
    <div className="flex items-center justify-center flex-shrink-0">
      <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" className="text-[#05073c]">
...
const managementItems = [
  {
    ...
    description: 'Administrá las ofertas laborales que creaste y visualizá sus postulaciones.',
  },
  {
    icon: UsersIcon,
    title: 'Candidatos',
  ...
  },
  {
    ...
    description: 'Visualizá las ofertas laborales cerradas y su historial de postulaciones.',
  },
  {
    icon: FileTextIcon,
    title: 'Borradores',
    count: '5 ofertas sin publicar',
    description: 'Continuá editando ofertas laborales que aún no fueron publicadas.',
  },
];
...
export const HomeReclutador = (): JSX.Element => {
  ...
  const { setCompanyName } = useReclutadorContext();
        ...
        <div className="w-full max-w-[1192px] mx-auto">
          ...
          {isLoading ? (
            <div className="text-center py-8">
              <p className="text-[#666666] text-[18px]">Cargando publicaciones...</p>
            </div>
          ) : jobs.length > 0 ? (
            ...
            <div className="text-center py-8">
              <p className="text-[#666666] text-[18px]">No hay publicaciones disponibles</p>
            </div>
          )}
        </div>
      </section>
