...
export const HomeCandidato: React.FC = () => {
  ...
  const getVisibleOptions = (section: { title: string; options: string[] }) => {
    return expandedSections[section.title] ? section.options : section.options.slice(0, 3);
  };
  ...
  const filteredAreaSuggestions = areaSuggestions.filter((s) =>
    s.toLowerCase().includes(areaInput.toLowerCase())
  );

  const filteredLocationSuggestions = locationSuggestions.filter((s) =>
  ...
  const filteredJobs = jobs.filter((job) => {
    const companyMatch =
      ...
      selectedFilters.empresa.some((c) => job.company_name === c);
    const jobTitleMatch =
      selectedFilters.puesto.length === 0 ||
      selectedFilters.puesto.some((t) => job.job_title === t);
    const locationMatch =
      selectedFilters.ubicación.length === 0 ||
      selectedFilters.ubicación.some((l) => job.location === l);
      ...
      <section className="flex w-full min-h-[200px] md:min-h-[240px] lg:min-h-[278px] flex-col items-center justify-center gap-4 md:gap-6 px-4 py-6 md:py-8 bg-[#1E2749]">
        ...
        <div className="flex flex-col w-full max-w-[90%] md:max-w-[600px] lg:max-w-[676px] relative">
          ...
          >
            ...
            />
            {areaInput && (
              <button onClick={handleAreaClear} className="flex-shrink-0 text-[#8c8c8c] hover:text-[#333333] transition-colors">
              ...
              <div className="absolute top-full left-0 right-0 bg-white border border-[#dedede] rounded-b-lg shadow-lg z-10 max-h-[300px] overflow-y-auto">
                {filteredAreaSuggestions.map((suggestion) => (
                  <button
                    ...
                    className="w-full px-4 md:px-8 lg:px-[60px] py-3 text-left hover:bg-gray-50 transition-colors font-normal text-[#333333] text-base tracking-[0] leading-[24px]"
                  >
                    {suggestion}
                  </button>
                ))}
          ...
          >
            ...
            />
            {locationInput && (
              <button onClick={handleLocationClear} className="flex-shrink-0 text-[#8c8c8c] hover:text-[#333333] transition-colors">
                <XIcon className="w-5 h-5" />
              </button>
            )}
            ...
            <>
              ...
              <div className="fixed inset-x-0 bottom-0 bg-white z-50 rounded-t-3xl shadow-2xl flex flex-col lg:hidden max-h-[85vh] transition-transform duration-300 ease-out">
                ...
                <div className="flex items-center justify-between px-6 py-4 border-b border-[#eeeeee]">
                  ...
                  <button onClick={() => setIsFilterOpen(false)} className="text-[#757575] hover:text-[#333333] transition-colors p-1">
