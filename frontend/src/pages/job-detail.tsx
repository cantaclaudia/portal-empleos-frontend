...
import { Menu as MenuIcon, Bone as XIcon, Home as HomeIcon, Search as SearchIcon, FileText as FileTextIcon, Settings as SettingsIcon, User as UserIcon, MapPin as MapPinIcon, Building as BuildingIcon, DollarSign as DollarSignIcon, ChevronLeft as ChevronLeftIcon, CheckCircle as CheckCircleIcon } from 'lucide-react';
...
export const JobDetail: React.FC = () => {
  ...
  useEffect(() => {
    const loadJob = async () => {
      try {
        ...
        const index = parseInt(jobIndex || '0', 10);
        if (result.data && result.data[index]) {
          setJob(result.data[index]);
        } else {
  ...
  const handleApply = async () => {
    ...
    setApplyResult(null);
    try {
      const jobOfferId = job.job_offer_id?.toString() ?? '';
      if (!jobOfferId) {
        setApplyResult({
          success: false,
          message: 'No se pudo identificar la oferta. El backend debe proporcionar job_offer_id en getAvailableJobs para habilitar la postulación.',
        });
        return;
      }
      ...
      setApplyResult({ success: true, message: 'Postulación realizada correctamente.' });
    } catch (err) {
      const message = err instanceof Error ? err.message : 'Error al postularse.';
      if (message.includes('0411') || message.toLowerCase().includes('not active')) {
        setApplyResult({ success: false, message: 'La oferta ya no se encuentra activa.' });
      } else {
          ...
          {loading ? (
            <Card className="bg-white border-0 shadow-sm">
              <CardContent className="flex flex-col items-center justify-center py-16">
                <p className="text-[#757575] text-xl">Cargando empleo...</p>
              </CardContent>
            </Card>
          ) : error ? (
            <Card className="bg-white border-0 shadow-sm">
            ...
            <div className="flex flex-col gap-6">
              ...
              <Card className="bg-white border-0 shadow-sm rounded-lg">
                <CardContent className="flex flex-col gap-3 px-6 md:px-8 py-6 md:py-8">
                  <h2 className="font-bold text-[#06083C] text-xl mb-2">Descripción</h2>
                  <p className="font-normal text-[#333333] text-base leading-relaxed">{job.job_description}</p>
                </CardContent>
              </Card>

              <Card className="bg-white border-0 shadow-sm rounded-lg">
                <CardContent className="flex flex-col gap-3 px-6 md:px-8 py-6 md:py-8">
                  <h2 className="font-bold text-[#06083C] text-xl mb-2">Requisitos</h2>
                  <p className="font-normal text-[#333333] text-base leading-relaxed">{job.requirements}</p>
                </CardContent>
              </Card>
              ...
              {applyResult && (
                <div className="max-w-[600px]">
                  {applyResult.success ? (
                    ...
                    <ErrorMessage message={applyResult.message} />
                  )}
                </div>
              )}
              ...
              <div className="flex justify-center pt-2">
                <Button
                  onClick={handleApply}
                  disabled={applying}
                  className="h-12 md:h-[56px] w-full max-w-[400px] rounded-lg bg-[#f46036] hover:bg-[#d9512e] px-6 py-3 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                >
